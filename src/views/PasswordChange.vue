        ...
        <header class="space">
          <supabase-function ref="childSupa" />
            <div class="register">
                ...
                <input type="password" v-model="nowPassword.value" placeholder="現在のパスワード">
                <div class="error">{{nowPassword.error_msg}}</div>

                <input type="password" v-model="newPassword.value" placeholder="新しいパスワード">
...
export default {
    ...
    methods: {
      async nextButton() {
        this.result=await this.$refs.childSupa.errorPasswordChangeCheck(this.nowPassword.value, this.newPassword.value, this.newPasswordConfirm.value);
        ...
        if(this.result.flag){
          this.$refs.childSupa.changePassword(this.$store.state.username, this.newPassword.value);
